...
package object dashboard {
  ...
  object DBIOResult {
    ...
    def fromFuture[A](failure: Throwable ⇒ ErrorResult)(fu: Future[A])(implicit ec: ExecutionContext): Result[A] =
      XorT[DBIO, ErrorResult, A](DBIO.from(fu map Xor.right recover {
        case e ⇒ Xor.left(failure(e))
      }))
