...
private[llectro] object Common {
  ...
  )(implicit http: HttpExt, executionContext: ExecutionContext, system: ActorSystem, materializer: Materializer, authToken: String): Future[Either[Errors, B]] = {
  ...
  def defaultFailure[B](implicit executionContext: ExecutionContext, materializer: Materializer): (StatusCode, ResponseEntity) ⇒ Future[Left[Errors, B]] =
    (status, entity) ⇒
