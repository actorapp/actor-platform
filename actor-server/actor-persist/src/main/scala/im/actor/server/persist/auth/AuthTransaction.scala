...
abstract class AuthTransactionBase[T](tag: Tag, tname: String) extends Table[T](tag, tname) {
...
class AuthTransactionTable(tag: Tag) extends AuthTransactionBase[models.AuthTransaction](tag, "auth_transactions") {
...
object AuthTransaction {
  ...
  def findChildren(transactionHash: String)(implicit ec: ExecutionContext): DBIO[Option[AuthTransactionChildren]] =
    for {
      email ← AuthEmailTransaction.find(transactionHash)
      phone ← AuthPhoneTransaction.find(transactionHash)
