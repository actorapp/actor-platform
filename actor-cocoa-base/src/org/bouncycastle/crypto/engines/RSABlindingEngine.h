//
//  Generated by the J2ObjC translator.  DO NOT EDIT!
//  source: /Users/ex3ndr/Develop/actor-model/actor-ios/build/java/org/bouncycastle/crypto/engines/RSABlindingEngine.java
//

#ifndef _OrgBouncycastleCryptoEnginesRSABlindingEngine_H_
#define _OrgBouncycastleCryptoEnginesRSABlindingEngine_H_

@class IOSByteArray;
@class JavaMathBigInteger;
@class OrgBouncycastleCryptoEnginesRSACoreEngine;
@class OrgBouncycastleCryptoParamsRSAKeyParameters;
@protocol OrgBouncycastleCryptoCipherParameters;

#include "J2ObjC_header.h"
#include "org/bouncycastle/crypto/AsymmetricBlockCipher.h"

@interface OrgBouncycastleCryptoEnginesRSABlindingEngine : NSObject < OrgBouncycastleCryptoAsymmetricBlockCipher > {
}

- (void)init__WithBoolean:(jboolean)forEncryption
withOrgBouncycastleCryptoCipherParameters:(id<OrgBouncycastleCryptoCipherParameters>)param OBJC_METHOD_FAMILY_NONE;

- (jint)getInputBlockSize;

- (jint)getOutputBlockSize;

- (IOSByteArray *)processBlockWithByteArray:(IOSByteArray *)inArg
                                    withInt:(jint)inOff
                                    withInt:(jint)inLen;

- (instancetype)init;

@end

J2OBJC_EMPTY_STATIC_INIT(OrgBouncycastleCryptoEnginesRSABlindingEngine)

CF_EXTERN_C_BEGIN
CF_EXTERN_C_END

J2OBJC_TYPE_LITERAL_HEADER(OrgBouncycastleCryptoEnginesRSABlindingEngine)

#endif // _OrgBouncycastleCryptoEnginesRSABlindingEngine_H_
