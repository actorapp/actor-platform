...
class UnreadWatcherSpec extends BaseAppSuite {
  ...
  it should "be no notifications when nothing is timed out" in tests.s.noNotifications()

  it should "add all subsequent messages from timed out dialog to notification" in tests.s.fromOneSender()

  it should "appear new message in notification after time out" in tests.s.fromTwoSenders()

  it should "be notifications for two users" in tests.s.twoNotifications()

  it should "be notifications for three users" in tests.s.threeNotifications()

  it should "be no notifications, when timeout setting is big" in tests.t.emptyWatcher()
  ...
  object tests {
    ...
    implicit val notifier = new DummyNotifier()
    ...
    object s {
      ...
      def noNotifications() = {
        whenReady(service.handleSendMessage(user12Peer, Random.nextLong(), TextMessage("Hello 1", Vector.empty, None)))(_ ⇒ ())
      ...
      def fromOneSender() = {
        ...
        whenReady(service.handleSendMessage(user12Peer, Random.nextLong(), TextMessage("Hello 2", Vector.empty, None)))(_ ⇒ ())
        whenReady(watcher.getNotifications) { notifications ⇒
          notifications should have length 1
      ...
      def fromTwoSenders() = {
        ...
        whenReady(service.handleSendMessage(user32Peer, Random.nextLong(), TextMessage("Hello 3", Vector.empty, None)))(_ ⇒ ())
        whenReady(watcher.getNotifications) { notifications ⇒
          notifications should have length 1
          notifications.head shouldEqual Notification(user2.id, Map(Some(user1.name) → 2))
        }
      ...
      def twoNotifications() = {
        ...
        whenReady(service.handleSendMessage(user21Peer, Random.nextLong(), TextMessage("hello to you", Vector.empty, None)))(_ ⇒ ())
        ...
        whenReady(service.handleSendMessage(user21Peer, Random.nextLong(), TextMessage("don't forget the milk!", Vector.empty, None)))(_ ⇒ ())
        whenReady(watcher.getNotifications) { notifications ⇒
          notifications should have length 2
      ...
      def threeNotifications() = {
        whenReady(service.handleSendMessage(user13Peer, Random.nextLong(), TextMessage("Where is the money!?", Vector.empty, None)))(_ ⇒ ())
        whenReady(futureSleep(4000).flatMap(_ ⇒ watcher.getNotifications)) { notifications ⇒
