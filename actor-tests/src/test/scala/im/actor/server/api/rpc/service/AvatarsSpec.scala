...
class AvatarsSpec extends BaseServiceSuite {
  ...
  private val validSmallAImg = Await.result(AsyncImage(validSmallBytes), 5.seconds)
  ...
  private val validLargeAImg = Await.result(AsyncImage(validLargeBytes), 5.seconds)
