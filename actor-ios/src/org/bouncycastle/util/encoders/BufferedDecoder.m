...
#include "org/bouncycastle/util/encoders/BufferedDecoder.h"
#include "org/bouncycastle/util/encoders/Translator.h"

@implementation OrgBouncycastleUtilEncodersBufferedDecoder
...
- (jint)processByteWithByte:(jbyte)inArg
  ...
  *IOSByteArray_GetRef(nil_chk(buf_), bufOff_++) = inArg;
  if (bufOff_ == buf_->size_) {
    resultLen = [((id<OrgBouncycastleUtilEncodersTranslator>) nil_chk(translator_)) decodeWithByteArray:buf_ withInt:0 withInt:buf_->size_ withByteArray:outArg withInt:outOff];
    bufOff_ = 0;
  }
  return resultLen;
}
...
- (jint)processBytesWithByteArray:(IOSByteArray *)inArg
  ...
  if (len > gapLen) {
    ...
    resultLen += [translator_ decodeWithByteArray:inArg withInt:inOff withInt:chunkSize withByteArray:outArg withInt:outOff];
    len -= chunkSize;
    inOff += chunkSize;
  }
  if (len != 0) {
...
- (void)copyAllFieldsTo:(OrgBouncycastleUtilEncodersBufferedDecoder *)other {
...
J2OBJC_CLASS_TYPE_LITERAL_SOURCE(OrgBouncycastleUtilEncodersBufferedDecoder)
