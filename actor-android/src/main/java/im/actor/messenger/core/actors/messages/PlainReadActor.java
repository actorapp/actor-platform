 ...
 */
public class PlainReadActor extends TypedActor<PlainReadInt> implements PlainReadInt {

    private static final TypedActorHolder<PlainReadInt> HOLDER = new TypedActorHolder<PlainReadInt>(
            PlainReadInt.class, PlainReadActor.class, "plain_reader");

    public static PlainReadInt plainRead() {
        return HOLDER.get();
    }

    public PlainReadActor() {
        super(PlainReadInt.class);
    }
    ...
    @Override
    public void markRead(int chatType, int chatId, long date) {
    ...
    private void peformMarkread(int chatType, int chatId, final long date) {
        ...
        if (chatType == DialogType.TYPE_USER) {
            ...
            ask(requests().messageRead(new OutPeer(PeerType.PRIVATE, chatId, userModel.getAccessHash()),
            ...
            ask(requests().messageRead(new OutPeer(PeerType.GROUP, chatId, groupModel.getAccessHash()),
                    date), new FutureCallback<ResponseVoid>() {
                @Override
                public void onResult(ResponseVoid result) {
                    Long pendingDate = pending.get(pendingUid);
